...
int main(void)
{
  ...
  while(1)
  {
    ...
    if( (elapsedTime = (millis() - startTime)) >= SAMPLE_INTERVAL_MS)
    {
      ...
      motorOutput = PID_balance(desiredAngle, presentAngle); //calls the PID function and sets the output to motorOutput. 

      left_desired_speed = motorOutput;
      right_desired_speed = motorOutput+90;
